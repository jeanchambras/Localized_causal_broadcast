...
/*
 * The encoder class defines an encoder and decoder for the message object.
 * To reduce network traffic instead of sending the full Message object we serialize its information into a byte array
 */
...
public class Encoder {
    ...
    byte[] encode(boolean ack, Message m) {
        ...
        b[7] = IntToByteArray(m.getMessageID())[0];
        b[8] = IntToByteArray(m.getMessageID())[1];
        b[9] = IntToByteArray(m.getMessageID())[2];
    ...
    /*
     * Convert a byte array to a tuple containing the ack flag (1: acknowledgment message; 0: normal message), and the
     * message
     */
    ...
    private byte[] intArray2byteArray(int[] src) {
    ...
    int[] byteArray2intArray(byte[] src) {
